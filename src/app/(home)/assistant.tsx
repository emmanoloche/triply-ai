...
/** The Assistant tab: a general travel chat. */
export default function Assistant() {
  ...
  // Opening the Assistant brings up the keyboard straight away. Tab screens stay
  // mounted, so this runs on every visit, not just the first. The short delay
  // lets the tab bar finish hiding before the keyboard opens, and leaving
  // closes the keyboard again. (useCallback is required by useFocusEffect's API.)
  useFocusEffect(
    useCallback(() => {
  ...
  // Loads the saved conversation once, when the screen first opens. Sending is
  // held back until it arrives, so an early message can't be overwritten by it.
  useEffect(() => {
    let cancelled = false;
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const res = await streamingFetch(`${origin}/api/assistant`, {
        method: "POST",
        headers: {
      ...
      if (conversation === conversationRef.current) setSending(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      <ScrollView
        ...
        onContentSizeChange={() => scrollRef.current?.scrollToEnd({ animated: !sending })}
      >
      ...
      <View
        ...
        style={{ borderTopWidth: 1, borderTopColor: INPUT_LINE, gap: 10 }}
      >
        <TextInput
          ref={inputRef}
          value={input}
        ...
        <Pressable
          ...
          style={{ backgroundColor: canSend ? CHAT_BLUE : SEND_DISABLED_BG }}
        >
          <SendArrow color={canSend ? "#FFFFFF" : "#9CA3AF"} />
        </Pressable>
      </View>
    </KeyboardAvoidingView>
  );

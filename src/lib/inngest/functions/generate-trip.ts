...
// The Gemini SDK throws with the raw API error body as `error.message`
...
export const generateTrip = inngest.createFunction(
  {
    ...
    // Gemini/Unsplash/DB failure doesn't silently cost the user a generation.
  ...
  async ({ event, step }) => {
    ...
    await step.run("set-status-generating", async () => {
      await db.update(trips).set({ status: "generating" }).where(eq(trips.id, data.tripId));
    });

    const generated = await step.run("call-gemini", async () => {
    ...
    // function's own `retries: 3` — the memoized "call-gemini" step above
    // won't re-run, so a cover-image retry doesn't cost another Gemini call.
